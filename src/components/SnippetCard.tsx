...
export const SnippetCard: React.FC<SnippetCardProps> = ({ snippet }) => {
  ...
  return (
    <div className="group glass rounded-xl overflow-hidden hover:border-primary/50 transition-all duration-300 flex flex-col h-full">
      {/* Header */}
      <div className="p-4 bg-dark-900/50 border-b border-white/5 flex items-center">
        {/* Sol - Title */}
        <div className="flex-1 flex items-center">
          <Link
            title={snippet.title}
            href={`/snippet/${snippet.id}`}
            className="p-1.5 flex items-center hover:bg-white/10 max-w-72 rounded-md transition-all text-slate-500 hover:text-white"
          >
